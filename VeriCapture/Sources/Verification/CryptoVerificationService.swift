...
class CryptoVerificationService {
    ...
    static func verifyMerkleProof(
        ...
        if treeSize == 1 && merkleProof.isEmpty && merkleIndex == 0 {
            // 単発の場合: MerkleRoot == LeafHash == EventHash
            let eventHashHex = eventHash.replacingOccurrences(of: "sha256:", with: "").lowercased()
            let rootHex = expectedRoot.replacingOccurrences(of: "sha256:", with: "").lowercased()
            return eventHashHex == rootHex
        }
        
        // バッチMerkle検証（通常のProof検証）
        var currentHash = eventHash.replacingOccurrences(of: "sha256:", with: "")
        ...
        for siblingHash in merkleProof {
            ...
            if index % 2 == 0 {
                currentHash = sha256(Data(verifyHexString: currentHash + sibling) ?? Data())
            } else {
                currentHash = sha256(Data(verifyHexString: sibling + currentHash) ?? Data())
            }
            index = index / 2
        }
    ...
    static func verifyTSAAnchor(
        ...
        // Step 1: 単発Merkle検証 (treeSize=1)
        let treeSize = anchor.treeSize ?? 1
        if treeSize == 1 {
            // 単発: MerkleRoot == LeafHash == EventHash
            let eventHashHex = eventHash.replacingOccurrences(of: "sha256:", with: "").lowercased()
            let rootHex = anchor.merkleRoot.replacingOccurrences(of: "sha256:", with: "").lowercased()
            
            if eventHashHex != rootHex {
                return .invalid(reason: "Single-leaf Merkle: MerkleRoot != EventHash")
            }
        } else {
            // バッチ: MerkleProof検証
