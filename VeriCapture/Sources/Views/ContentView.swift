...
struct CaptureResultView: View {
    ...
    private func handleInternalExport(includeLocation: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            Task {
                // 法務用エクスポート（メディア + Proof JSON をZIPにまとめて）
                if let zipURL = await viewModel.exportInternalProof(eventId: result.eventId, includeLocation: includeLocation) {
                    await MainActor.run {
                        viewModel.isSharing = false
                        // ZIPファイルを共有
...
struct EventDetailView: View {
    ...
    private func handleInternalExport(includeLocation: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            Task {
                // 法務用エクスポート（メディア + Proof JSON をZIPにまとめて）
                if let zipURL = await viewModel.exportInternalProof(eventId: event.eventId, includeLocation: includeLocation) {
                    await MainActor.run {
                        viewModel.isSharing = false
                        // ZIPファイルを共有
                        presentShareSheet(items: [zipURL])
