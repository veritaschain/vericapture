...
/// 共有オプション選択シート
/// - 画像: QR付き / 画像のみ / エクスポート / C2PA（4オプション）
/// - 動画: 動画のみ / エクスポート / C2PA（3オプション）
struct ShareOptionsSheet: View {
    ...
    let onInternalExport: (Bool) -> Void
    let onRawDataExport: (() -> Void)?
    let onC2PAExport: (() -> Void)?
    ...
    init(
        ...
        onInternalExport: @escaping (Bool) -> Void,
        ...
        NavigationView {
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    // ===== オプション4: C2PA互換エクスポート =====
                    C2PAExportSection(showInfo: $showC2PAInfo) {
                        authenticateAndC2PAExport()
                    }
    ...
    private func authenticateAndC2PAExport() {
        Task {
            let result = await BiometricAuthService.shared.authenticate(for: .exportFullProof)
...
/// C2PA互換エクスポートセクション（ⓘボタン付き）
private struct C2PAExportSection: View {
        ...
        Button(action: action) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(L10n.Share.c2paExportDesc)
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .lineLimit(2)
                }
