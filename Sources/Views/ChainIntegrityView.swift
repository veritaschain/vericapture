...
struct ChainIntegrityView: View {
    ...
    var body: some View {
        List {
            ...
            // チェーンリセットセクション（エラーがある場合のみ表示）
            if let result = verificationResult, !result.errors.isEmpty {
                Section {
            ...
            Section {
                Button {
                    ...
                    HStack {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text(L10n.Export.fullChainTitle)
                                .foregroundColor(.primary)
                            Text(L10n.Export.fullChainDesc)
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                ...
                .disabled(isExporting || statistics?.totalEvents == 0)
            } header: {
                HStack {
                    Text(L10n.Export.fullChainTitle)
                    if let stats = statistics, stats.tombstoneCount > 0 {
                        Text(L10n.Export.tombstoneCount(stats.tombstoneCount))
                            .font(.caption2)
                            .foregroundColor(.orange)
                    }
                }
            } footer: {
                Text(L10n.Export.tombstoneNote)
            }
        }
        ...
        .alert(L10n.Export.packageGenerated, isPresented: $showExportSuccess) {
            Button(L10n.Common.ok, role: .cancel) {}
        } message: {
            if let stats = statistics {
                Text(L10n.Export.packageIncludes(stats.totalEvents, stats.tombstoneCount))
            }
        }
    ...
    private func exportFullChain() {
        isExporting = true
        Task {
            do {
                let url = try StorageService.shared.exportFullChainAsZip(includeLocation: false, includeShareable: true)
                await MainActor.run {
                ...
                print("[ChainIntegrityView] Export error: \(error)")
                await MainActor.run {
                    self.isExporting = false
                }
            }
