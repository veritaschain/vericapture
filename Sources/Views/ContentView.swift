    ...
    var body: some View {
        VStack(spacing: 32) {
            ...
            Button {
                ...
                HStack {
                    Image(systemName: "camera.fill")
                    Text(L10n.Permission.allowCamera)
                }
                .font(.headline)
                .foregroundColor(.white)
...
struct CaptureResultView: View {
            ...
            .sheet(isPresented: $showShareOptions) {
                if let image = result.image {
                    ShareOptionsSheet(
                        ...
                        onInternalExport: { includeLocation in
                            handleInternalExport(includeLocation: includeLocation)
                        },
                        onRawDataExport: {
                            handleRawDataExport()
                        }
    ...
    private func handleInternalExport(includeLocation: Bool) {
        viewModel.isSharing = true
        showShareOptions = false
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            Task {
                if let url = await viewModel.exportInternalProof(eventId: result.eventId, includeLocation: includeLocation) {
                    var items: [Any] = [url]
                    if let image = result.image {
                        // 参照用プレビューの透かしを追加
                        let watermarkedImage = ReferenceWatermark.addWatermark(to: image)
    ...
    private func handleRawDataExport() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            Task {
                // 生データ（元画像 + Proof JSON）をエクスポート
                if let urls = await viewModel.exportRawData(eventId: result.eventId) {
                    await MainActor.run {
                        viewModel.isSharing = false
                        // ファイルアプリに保存するActivityViewControllerを表示
...
struct EventDetailView: View {
    ...
    private func loadImage() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let img = StorageService.shared.loadImage(eventId: event.eventId)
            DispatchQueue.main.async {
                loadedImage = img
            }
        }
    ...
    private func handleInternalExport(includeLocation: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            Task {
                if let url = await viewModel.exportInternalProof(eventId: event.eventId, includeLocation: includeLocation) {
                    var items: [Any] = [url]
                    if let image = loadedImage {
                        // 参照用プレビューの透かしを追加
                        let watermarkedImage = ReferenceWatermark.addWatermark(to: image)
    ...
    private func handleRawDataExport() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            Task {
                // 生データ（元画像 + Proof JSON）をエクスポート
                if let urls = await viewModel.exportRawData(eventId: event.eventId) {
                    await MainActor.run {
                        viewModel.isSharing = false
                        // ファイルアプリに保存するActivityViewControllerを表示
                        presentShareSheet(items: urls)
...
struct SettingsView: View {
        ...
        NavigationView {
            List {
                    ...
                    Button {
                        ...
                        HStack {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                ...
                                Text(L10n.Settings.exportAllForensicDesc)
                                    .font(.caption)
                                    .foregroundColor(.secondary)
                            }
                            Spacer()
                ...
                Section(L10n.Settings.specTitle) {
                    Link(destination: URL(string: "https://veritaschain.org/vap/cpp/vericapture/")!) {
                        HStack {
                    ...
                    Link(destination: URL(string: "https://github.com/veritaschain/cpp-spec/blob/main/docs/VeriCapture/VeriCapture_WorldFirst_Report_Final_EN.md")!) {
                        HStack {
    ...
    private func exportAllInternalProofsCore() async {
        ...
        do {
            ...
            // ZIPファイルを作成
            let zipURL = try StorageService.shared.exportAllInternalProofsAsZip(proofs: proofs)
