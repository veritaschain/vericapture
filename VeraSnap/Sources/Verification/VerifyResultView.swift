...
struct VerifyResultView: View {
    ...
    private var statusHeader: some View {
        VStack(spacing: 16) {
            ...
            // 検証失敗時の警告メッセージ
            if result.overallStatus == .signatureInvalid || result.overallStatus == .hashMismatch || result.overallStatus == .assetMismatch {
                HStack(spacing: 8) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundColor(.orange)
                    Text(L10n.Verify.tamperWarning)
                        .font(.caption)
                        .foregroundColor(.secondary)
    ...
    private var conclusionText: String {
        ...
        case .verified, .anchorVerified:
            // 動画の場合は動画用メッセージ
            return result.isVideo ? L10n.Verify.resultAuthenticVideoMessage : L10n.Verify.resultAuthenticMessage
    ...
    private var proofDetailsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(spacing: 0) {
                ...
                // メディアタイプ（画像/動画）v42
                let mediaTypeIcon = result.isVideo ? "video.fill" : "photo.fill"
                let mediaTypeLabel = result.isVideo ? L10n.Verify.mediaTypeVideo : L10n.Verify.mediaTypeImage
                detailRow(icon: mediaTypeIcon, label: L10n.Verify.mediaType, value: mediaTypeLabel)
    ...
    private var footerSection: some View {
        VStack(spacing: 8) {
            Text(L10n.Verify.verifiedByVeraSnap)
                .font(.caption)
                .foregroundColor(.secondary)
