...
//  VeraSnap
//
//  Verification Tab View - Integrated VeraCheck
        ...
        NavigationStack {
            ...
            .sheet(isPresented: $showJSONInput, onDismiss: {
                ...
                if let json = pendingJSONInput {
                    ...
                    // State更新を確実に反映させるため、十分な待機時間を設ける
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                        pendingProofJSON = json
                        // State更新が確実に反映されてから画像選択モードへ遷移
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
            ...
            .sheet(isPresented: $showImageVerification, onDismiss: {
                ...
                ImageVerificationView(
                    proofJSON: Binding(
                        get: { pendingProofJSON ?? "" },
                        set: { pendingProofJSON = $0 }
                    ),
...
"""
    ...
    private func showInvalidQRAlert(url: String) {
        let alert = UIAlertController(
            title: "無効なQRコード",
            message: "VeraSnap形式のQRコードではありません。\n\nURL: \(url)",
            preferredStyle: .alert
        )
...
struct ImageVerificationView: View {
    ...
    /// ProofJSONから動画かどうかを判定
    private var isVideo: Bool {
        // JSONのスペース有無に関わらず検出
        // Pretty-printed JSON: "AssetType" : "VIDEO"
        // Compact JSON: "AssetType":"VIDEO"
        proofJSON.contains("\"AssetType\"") && proofJSON.contains("\"VIDEO\"")
    }
    
    /// タイトル（動画/画像で切り替え）
    private var verificationTitle: String {
        isVideo ? L10n.Verify.mediaVerificationTitle : L10n.Verify.imageVerificationTitle
    }
    
    /// 説明文（動画/画像で切り替え）
    private var verificationDesc: String {
        isVideo ? L10n.Verify.mediaVerificationDesc : L10n.Verify.imageVerificationDesc
    }
    
    /// 選択タイトル（動画/画像で切り替え）
    private var selectTitle: String {
        isVideo ? L10n.Verify.selectMediaTitle : L10n.Verify.selectImageToVerify
    }
    
    /// 選択ヒント（動画/画像で切り替え）
    private var selectHint: String {
        isVideo ? L10n.Verify.selectMediaFromFiles : L10n.Verify.selectImageFromFiles
    }
    
    /// アイコン（動画/画像で切り替え）
    private var iconName: String {
        isVideo ? "video.badge.checkmark" : "photo.badge.checkmark"
    }
        ...
        NavigationStack {
            VStack(spacing: 20) {
                    ...
                    VStack(spacing: 12) {
                        ...
                        HStack {
                            ...
                            VStack(alignment: .leading) {
                                Text(isVideo ? L10n.Verify.mediaSelected : L10n.Verify.imageSelected)
                                    .font(.subheadline)
                                    .foregroundColor(.secondary)
                        ...
                        Button {
                            ...
                            Text(isVideo ? L10n.Verify.changeMedia : L10n.Verify.changeImage)
                                .font(.caption)
                                .foregroundColor(.blue)
                        }
                    }
                ...
                VStack(spacing: 12) {
                    ...
                    Button {
                        ...
                        HStack {
                            ...
                            Text(selectedImageData != nil 
                                 ? (isVideo ? L10n.Verify.verifyWithVideo : L10n.Verify.verifyWithImage)
                                 : (isVideo ? L10n.Verify.verifyProofOnlyVideo : L10n.Verify.verifyProofOnly))
                        }
                        .font(.headline)
                        .foregroundColor(.white)
                        .frame(maxWidth: .infinity)
                    ...
                    // 注意事項
                    Text(isVideo ? L10n.Verify.videoVerificationNote : L10n.Verify.imageVerificationNote)
                        .font(.caption2)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal)
