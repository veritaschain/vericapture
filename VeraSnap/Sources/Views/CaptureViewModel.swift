...
final class CaptureViewModel: ObservableObject {
    ...
    init() {
        ...
        // 1分自動停止時の動画結果を監視
        cameraService.$autoStoppedVideoResult
            .receive(on: RunLoop.main)
            .compactMap { $0 }  // nilを除外
            .sink { [weak self] videoResult in
                Task { @MainActor [weak self] in
                    await self?.handleAutoStoppedVideo(videoResult)
                }
            }
            .store(in: &cancellables)
    }
    ...
    /// 1分自動停止時の動画を処理
    private func handleAutoStoppedVideo(_ videoResult: VideoCaptureResult) async {
        print("[CaptureViewModel] 📢 Handling auto-stopped video recording")
        
        // 結果を消費済みとしてクリア
        cameraService.autoStoppedVideoResult = nil
        ...
        do {
            ...
            // Attested Capture: 認証情報を構築（録画開始時に取得したもの）
            var humanAttestation: HumanAttestation? = nil
            if let authResult = videoAuthResult {
                humanAttestation = eventBuilder.buildHumanAttestation(
                    from: authResult,
                ...
                print("[CaptureViewModel] Video attestation (auto-stop): verified=\(authResult.success)")
            }
            ...
            if SubscriptionService.shared.canSaveProof(currentCount: currentProofCount) {
                ...
                print("[CaptureViewModel] ✅ Auto-stopped video saved: \(event.eventId), duration: \(String(format: "%.1f", videoResult.duration))s")
            } else {
                // 上限到達
                showLimitReached = true
                ...
                print("[CaptureViewModel] Video recording limit reached (auto-stop)")
            }
            
        } catch {
            errorMessage = error.localizedDescription
            showError = true
            print("[CaptureViewModel] Failed to save auto-stopped video: \(error)")
        }
    ...
    func exportInternalProof(eventId: String, includeLocation: Bool = false, includeC2PA: Bool = false) async -> URL? {
        do {
            ...
            let folderName = "VeraSnap_Forensic_\(timestamp)"
    ...
    func exportRawData(eventId: String) async -> URL? {
        do {
            ...
            // 一時ディレクトリにフォルダを作成
            let timestamp = ISO8601DateFormatter().string(from: Date()).replacingOccurrences(of: ":", with: "-")
            let folderName = "VeraSnap_\(timestamp)"
            let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent(folderName)
            try? FileManager.default.removeItem(at: tempDir)
            try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
