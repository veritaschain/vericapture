...
struct PaywallView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if subscriptionService.isLoading {
                ...
                VStack(spacing: 16) {
                    ...
                    // 詳細エラーメッセージを表示（デバッグ用）
                    if let errorDetail = subscriptionService.errorMessage {
                        Text(errorDetail)
                            .font(.caption)
                            .foregroundColor(.red)
                            .multilineTextAlignment(.center)
    ...
    private var purchaseButton: some View {
        let isDisabled = selectedProduct == nil || isPurchasing || subscriptionService.isLoading
        ...
        return Button {
            ...
            .background(
                LinearGradient(
                    colors: isDisabled ? [.gray, .gray.opacity(0.8)] : [.blue, .purple],
                    startPoint: .leading,
                    endPoint: .trailing
                )
            )
