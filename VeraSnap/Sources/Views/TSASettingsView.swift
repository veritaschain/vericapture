    ...
    var body: some View {
        List {
            Section {
                ...
                Text(L10n.TSASettings.configuredProviders)
            } footer: {
                Text(L10n.TSASettings.reorderHint)
            }
...
struct AddTSAProviderView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                Section {
                    TextField(L10n.TSASettings.providerName, text: $name)
                    TextField(L10n.TSA.endpointUrl, text: $endpoint)
                        .keyboardType(.URL)
                        .autocapitalization(.none)
                }
                
                Section {
                    Picker(L10n.TSASettings.region, selection: $selectedRegion) {
                    ...
                    Toggle(L10n.TSA.eidasQualified, isOn: $isEidasQualified)
                }
                
                Section {
                    Text(L10n.TSASettings.customProviderHint)
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            }
            .navigationTitle(L10n.TSASettings.addProvider)
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .cancellationAction) {
                    Button(L10n.TSASettings.cancel) {
                        dismiss()
                    }
                }
                ToolbarItem(placement: .confirmationAction) {
                    Button(L10n.TSASettings.add) {
                        addProvider()
                    }
                    .disabled(!isValid)
                }
