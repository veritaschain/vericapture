...
// MARK: - SignerInfo (署名者情報 - CPP v1.0拡張)
// NOTE: この情報はEventHashの計算対象に含まれ、改ざん検出が可能

struct SignerInfo: Codable, Sendable {
    let name: String           // 署名者名
    let identifier: String?    // 識別子（オプション、デバイスID等）
...
struct CPPEvent: Codable, Sendable, Identifiable {
    ...
    let signerInfo: SignerInfo?  // 署名者情報（監査用）
...
struct Asset: Codable, Sendable {
    ...
    let assetSize: Int
    let mimeType: String
    let videoMetadata: VideoMetadata?  // 動画の場合のみ
...
struct VideoMetadata: Codable, Sendable {
    let duration: Double           // 秒数
    let resolution: VideoResolution
    let frameRate: Double          // FPS
    let codec: String              // h264, hevc等
    let hasAudio: Bool
...
struct VideoResolution: Codable, Sendable {
    let width: Int
    let height: Int
...
// - TSAに投げる値は AnchorDigest のみ
// - AnchorDigest = MerkleRoot (32 bytes hex)
// - TSAResponse.messageImprint.hashedMessage == AnchorDigest であること必須

struct AnchorRecord: Codable, Sendable {
    let anchorId: String
    let anchorType: String            // "RFC3161" 固定
    
    // --- Merkle Tree ---
    let merkleRoot: String            // sha256:xxx 形式
    let treeSize: Int                 // ツリーのリーフ数（単発=1）
    let eventCount: Int               // アンカー対象イベント数（=treeSize）
    let firstEventId: String
    let lastEventId: String
    
    // --- TSA Anchor ---
    let anchorDigest: String          // TSAに投げた値 = MerkleRoot hex (32 bytes)
    let anchorDigestAlgorithm: String // "sha-256" 固定
    let tsaToken: Data?               // RFC3161 TimeStampToken (DER format)
    let tsaMessageImprint: String?    // TSAから返されたmessageImprint.hashedMessage (hex)
    
    // --- Metadata ---
    let timestamp: String
    let anchorProof: String?          // 旧互換
    ...
    /// TSA messageImprint検証: AnchorDigest と一致すること
    var isMessageImprintValid: Bool {
        guard let imprint = tsaMessageImprint else { return false }
        return imprint.lowercased() == anchorDigest.lowercased()
    }
}
...
struct ShareableEventInfo: Codable, Sendable {
    ...
    let timestamp: String
    let assetHash: String
    let assetType: String?      // v42: VIDEO or IMAGE
    let assetName: String?      // v42: Original filename
...
struct TimestampProofInfo: Codable, Sendable {
    let type: String
    let issuedAt: String
    let token: String        // RFC3161 TimeStampToken (DER/base64)
    let merkleRoot: String?  // MerkleRoot (sha256:...)
    let tsaService: String?  // TSA Service endpoint
    
    // v42.2 (CPP v1.2): TSAアンカリング仕様の明確化
    let treeSize: Int?               // ツリーのリーフ数（単発=1）
    let anchorDigest: String?        // TSAに投げた値 (hex, 32 bytes)
    let digestAlgorithm: String?     // "sha-256" 固定
    let messageImprint: String?      // TSAから返されたmessageImprint.hashedMessage
    
    // v42.3: 各イベントのMerkle Tree内位置
    let merkleIndex: Int?            // イベントのリーフインデックス
    let merkleProof: [String]?       // Merkle Proof（兄弟ハッシュの配列）
...
struct AnchorInfo: Codable, Sendable {
    ...
    /// messageImprint検証: AnchorDigest と一致すること
    var isMessageImprintValid: Bool {
        guard let imprint = tsaMessageImprint else { return false }
        return imprint.lowercased() == anchorDigest.lowercased()
    }
