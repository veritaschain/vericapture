...
final class StorageService: @unchecked Sendable {
    ...
    func initialize() throws {
        let dbPath = getDocumentsDirectory().appendingPathComponent("verasnap.db").path
    ...
    private func migrateTablesIfNeeded() throws {
        ...
        let columnsToAdd = [
            ...
            // v42.3: 各イベントのMerkle Tree内位置を保存
            ("merkle_index", "INTEGER DEFAULT 0"),
    ...
    func updateEventAnchor(eventId: String, anchorId: String, merkleIndex: Int = 0, merkleProof: [String] = []) throws {
        lock.lock()
        defer { lock.unlock() }
        
        // Convert merkle proof array to JSON string
        let proofJson: String
        if merkleProof.isEmpty {
            proofJson = "[]"
        } else {
            let encoder = JSONEncoder()
            if let data = try? encoder.encode(merkleProof),
        ...
        let update = "UPDATE events SET anchor_id = ?, merkle_index = ?, merkle_proof = ? WHERE event_id = ?"
        
        var statement: OpaquePointer?
        if sqlite3_prepare_v2(db, update, -1, &statement, nil) == SQLITE_OK {
            sqlite3_bind_text(statement, 1, anchorId, -1, SQLITE_TRANSIENT)
            sqlite3_bind_int(statement, 2, Int32(merkleIndex))
            sqlite3_bind_text(statement, 3, proofJson, -1, SQLITE_TRANSIENT)
            sqlite3_bind_text(statement, 4, eventId, -1, SQLITE_TRANSIENT)
        ...
        print("[Storage] Event \(eventId) anchored: index=\(merkleIndex), proof=\(merkleProof.count) siblings")
    }
    ...
    /// イベントのMerkle Tree内位置情報を取得
    struct EventMerkleInfo: Sendable {
        let merkleIndex: Int
        let merkleProof: [String]
    }
    
    func getEventMerkleInfo(eventId: String) -> EventMerkleInfo? {
        lock.lock()
        defer { lock.unlock() }
        
        let query = "SELECT merkle_index, merkle_proof FROM events WHERE event_id = ?"
        ...
        if sqlite3_prepare_v2(db, query, -1, &statement, nil) == SQLITE_OK {
            ...
            if sqlite3_step(statement) == SQLITE_ROW {
                let merkleIndex = Int(sqlite3_column_int(statement, 0))
                
                // Parse merkle_proof JSON
                var merkleProof: [String] = []
                if let proofText = sqlite3_column_text(statement, 1) {
                    let proofString = String(cString: proofText)
                    if let data = proofString.data(using: .utf8),
                ...
                sqlite3_finalize(statement)
                return EventMerkleInfo(merkleIndex: merkleIndex, merkleProof: merkleProof)
            }
        }
        sqlite3_finalize(statement)
        return nil
    ...
    func exportShareableProof(eventId: String) throws -> URL {
        ...
        let merkleInfo = getEventMerkleInfo(eventId: eventId)
        ...
        let shareableProof = InternalProofJSON(
            ...
            anchor: anchor.flatMap { anc -> AnchorInfo? in
                ...
                return AnchorInfo(
                    ...
                    merkleProof: merkleInfo?.merkleProof ?? [],
                    merkleIndex: merkleInfo?.merkleIndex ?? 0,
    ...
    func exportAllInternalProofsAsZip(proofs: [InternalProofJSON]) throws -> URL {
        ...
        // 一時ディレクトリを作成
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("verasnap_export_\(UUID().uuidString)")
        ...
        let zipFilename = "VeraSnap_Export_\(timestamp).zip"
    ...
    func exportForensicPackageAsZip(proofs: [InternalProofJSON], signerName: String?, includeMedia: Bool = false, includeC2PA: Bool = false) throws -> URL {
        ...
        // 一時ディレクトリを作成
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("verasnap_forensic_\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        ...
        dateFormatter.dateFormat = "yyyyMMdd_HHmmss"
        let timestamp = dateFormatter.string(from: Date())
        let zipFilename = "VeraSnap_Forensic_\(timestamp).zip"
        let zipPath = FileManager.default.temporaryDirectory.appendingPathComponent(zipFilename)
    ...
    private func generateForensicReadme(proofCount: Int, tombstoneCount: Int, mediaCount: Int, videoCount: Int = 0, c2paCount: Int = 0, signerName: String?) -> String {
        ...
        var readme = """
        ================================================================================
        VeraSnap - Forensic Export Package
        ================================================================================
        
        Generated: \(timestamp)
        App Version: VeraSnap/1.0
        ...
        https://veritaschain.org/vap/cpp/verasnap
    ...
    func exportShareablePackage() throws -> URL {
        ...
        dateFormatter.dateFormat = "yyyyMMdd_HHmmss"
        let timestamp = dateFormatter.string(from: Date())
        let filename = "VeraSnap_Shareable_\(timestamp).json"
        let filePath = exportDirectory.appendingPathComponent(filename)
        try data.write(to: filePath)
    ...
    func exportInternalPackage(includeLocation: Bool = false) throws -> URL {
        ...
        dateFormatter.dateFormat = "yyyyMMdd_HHmmss"
        let timestamp = dateFormatter.string(from: Date())
        let filename = "VeraSnap_Internal_\(timestamp).json"
        let filePath = exportDirectory.appendingPathComponent(filename)
        try data.write(to: filePath)
    ...
    func exportFullChainAsZip(includeLocation: Bool = false, includeShareable: Bool = true) throws -> URL {
        // 一時ディレクトリを作成
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent("verasnap_full_export_\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        ...
        dateFormatter.dateFormat = "yyyyMMdd_HHmmss"
        let timestamp = dateFormatter.string(from: Date())
        let zipFilename = "VeraSnap_FullExport_\(timestamp).zip"
        let zipPath = FileManager.default.temporaryDirectory.appendingPathComponent(zipFilename)
    ...
    private func buildShareableProof(event: CPPEvent) throws -> ShareableProofJSON {
        ...
        let merkleInfo = getEventMerkleInfo(eventId: event.eventId)
        let publicKeyBase64 = CryptoService.shared.getPublicKeyBase64()
        ...
        return ShareableProofJSON(
            ...
            timestampProof: anchor.flatMap { anc -> TimestampProofInfo? in
                ...
                return TimestampProofInfo(
                    ...
                    merkleIndex: merkleInfo?.merkleIndex,  // イベントのリーフインデックス
                    merkleProof: merkleInfo?.merkleProof   // Merkle Proof
                )
            },
    ...
    private func generateExportReadme(includeLocation: Bool, includeShareable: Bool) -> String {
        ...
        Contents:
        ---------
        \(includeShareable ? "- VeraSnap_Shareable_*.json: Third-party verification data (minimal, privacy-first)" : "")
        - VeraSnap_Internal_*.json: Full audit trail (forensic, includes tombstones)
        ...
        Verification:
        -------------
        To verify proofs, scan the QR code or use the VeraCheck feature in the app.
...
extension StorageService {
    ...
    func exportCase(caseId: String, includeLocation: Bool = false) throws -> URL {
        ...
        let fileName = "VeraSnap_Case_\(caseItem.name.replacingOccurrences(of: " ", with: "_"))_\(Int(Date().timeIntervalSince1970)).json"
    ...
    func exportCaseAsZip(caseId: String, includeLocation: Bool = false) throws -> URL {
        ...
        let packageName = "VeraSnap_\(caseItem.name.replacingOccurrences(of: " ", with: "_"))_\(dateFormatter.string(from: Date()))"
        ...
        Verification:
        Each photo can be verified using VeraSnap app or VeraCheck web tool.
