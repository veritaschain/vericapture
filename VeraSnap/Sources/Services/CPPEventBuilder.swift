...
final class CPPEventBuilder {
    ...
    func buildIngestEvent(
        ...
        let assetId = "urn:cpp:asset:verasnap:\(UUIDv7.generate())"
    ...
    func generateProofJSON(event: CPPEvent, anchor: AnchorRecord?, locationInfo: LocationInfo? = nil, signerName: String? = nil) -> ProofJSON {
        ...
        if let anchor = anchor, anchor.status == .completed {
            ...
            // Merkle情報をデータベースから取得
            let merkleInfo = StorageService.shared.getEventMerkleInfo(eventId: event.eventId)
            ...
            anchorInfo = AnchorInfo(
                ...
                treeSize: anchor.treeSize,  // リーフ数
                anchorDigest: anchor.anchorDigest,  // TSAに投げた値
                anchorDigestAlgorithm: anchor.anchorDigestAlgorithm,
                tsaResponse: tsaTokenBase64,  // RFC3161 TimeStampToken (DER/base64)
                tsaMessageImprint: anchor.tsaMessageImprint,  // TSAから返されたmessageImprint
        ...
        return ProofJSON(
            proofVersion: "1.1",  // CPP v1.2 (v42.2)
